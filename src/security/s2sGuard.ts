...
function toStringHeader(value: string | string[] | undefined): string | undefined {
...
function parseSignature(input: string): Buffer {
...
function bodySha256(rawBody: Buffer | undefined): string {
  const digest = createHash('sha256');
  digest.update(rawBody ?? Buffer.from(''));
  return digest.digest('hex');
}

function buildBaseString(request: FastifyRequest, timestamp: string, nonce: string): string {
...
function isTimestampFresh(timestampSec: number, skewSec: number): boolean {
  const now = Math.floor(Date.now() / 1000);
  return Math.abs(now - timestampSec) <= skewSec;
}

export function createS2SGuard(config: AppConfig) {
  const nonceStore = new NonceStore(config.s2s.nonceTtlSec);

  return async function verify(request: FastifyRequest): Promise<void> {
